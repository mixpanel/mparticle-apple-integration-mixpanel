...
VERSION="$1"
REPO="${2:-mixpanel/mparticle-apple-integration-mixpanel}"

if [ -z "$VERSION" ]; then
  echo "Usage: $0 <version> [repo]"
  echo "Example: $0 1.0.0 mixpanel/mparticle-apple-integration-mixpanel"
  exit 1
fi
...
TAG_PREFIX=$(jq -e -r '.["mparticle-mixpanel"].tag_prefix' "$MODULES_JSON" 2>/dev/null || echo "INVALID")
